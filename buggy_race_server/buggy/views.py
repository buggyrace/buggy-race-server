...
def handle_uploaded_json(form, user, is_api=False):
    ...
    if (form.is_submitted() and form.validate()) or is_api:
        ...
        duplicate_properties_reported = set()
        def _check_for_duplicate_properties(incoming_data_pairs):
            dedupped_dic = {}
            for (key, value) in incoming_data_pairs:
                if not is_api and key in dedupped_dic:
                    if key not in duplicate_properties_reported:
                        flash(
                            f"JSON contains multiple definitions for {key}, "
                            "ignoring all but last value",
                            "warning"
                        )
                        duplicate_properties_reported.add(key)
                dedupped_dic[key] = value
            return dedupped_dic

        try:
            dirty_buggy_data = json.loads(
                form.buggy_json.data,
                object_pairs_hook=_check_for_duplicate_properties
            )
        ...
        clean_buggy_data = {}
        word_too = ""
        if type(dirty_buggy_data) == list:
            if is_api:
                return {"error": "JSON is a list, but should be a JSON object"}
            flash(
                "Your JSON should be a single JSON object, with property names, not a list. "
                "Maybe you tried to upload more than one buggy?",
                "danger"
            )
        elif type(dirty_buggy_data) != dict:
            if dirty_buggy_data is None:
                if is_api:
                    return {"error": "JSON is null, but should be a JSON object"}
                flash(
                    "Your JSON should be a single JSON object, with property names, not \"null\". "
                    "This doesn't describe a buggy!",
                    "danger"
                )
            else:
                if is_api:
                    return {"error": "JSON is a value, but should be a JSON object"}
                flash(
                    "Your JSON should be a single JSON object, with property names, not a value. "
                    "This doesn't describe a buggy!",
                    "danger"
                )
        else: # it's a dict, so we can iterate through the keys
            for key in dirty_buggy_data:
                key_type = type(key)
                if key_type != str:
                    if not is_api:
                        flash(
                            "Make sure your buggy JSON only contains properties which are strings: "
                            f"ignoring {key_type}",
                            "warning"
                        )
                    ...
                    try:
                        ...
                        if not is_api:
                            flash(
                                "Value for id was ignored because it wasn't an integer",
                                "warning"
                            )
                    ...
                    if Buggy.DEFAULTS[key] == False and isinstance(Buggy.DEFAULTS[key], bool):
                        if isinstance(dirty_buggy_data[key], bool):
                            ...
                            if dirty_buggy_data[key] == 'true':
                                ...
                                if not is_api:
                                    flash(
                                        f"Value for {key} was ignored because it wasn't true or false",
                                        "warning"
                                    )
                            if not is_api and was_ok_boolean:
                                flash(
                                    f"Value for {key} wasn't a JSON boolean, but OK: "
                                    f"\"{dirty_buggy_data[key]}\" accepted as {str(clean_buggy_data[key]).lower()}",
                                    "info"
                                )
                        ...
                        try:
                            ...
                            if not is_api:
                                flash(
                                    f"Value for {key} was ignored because it wasn't an integer",
                                    "warning"
                                )
                    else:
                        dirty_buggy_data[key] = str(dirty_buggy_data[key]).strip().lower()
                        ...
                        if s == "":
                            if not is_api:
                                flash(
                                    f"Value for {key} was ignored because it didn't have "
                                    "any alphanumeric characters in it",
                                    "warning"
                                )
                        else:
                            # check lengths:
                            if max_str_len := Buggy.STRING_COL_LENGTH.get(key):
                                if len(s) > max_str_len:
                                    ...
                                    if not is_api:
                                        flash(
                                            f"Value for {key} was truncated to {max_str_len} characters",
                                            "warning"
                                        )
                        if key in Buggy.GAME_DATA:
                            if s not in Buggy.GAME_DATA[key]:
                                if not is_api:
                                    flash(
                                        f"Value for {key} was ignored because \"{s}\" is not a valid choice"
                                        "warning"
                                    )
                            else:
                    ...
                    if not is_api:
                        flash(
                            f"Unrecognised setting \"{key}\" was ignored {word_too}",
                            "warning"
                        )
                        word_too = "too"
        qty_defaults = 0
        ...
        if qty_explicits == 0:
            ...
            if qty_defaults > 0:
                if not is_api:
                    ...
                    flash(
                        f"{qty_defaults} setting{s} {was} not specified and got "
                        f"default value{s} instead",
                        "info"
                    )
            if 'buggy_id' not in clean_buggy_data:
                clean_buggy_data['buggy_id'] = 1 # force this as a default
